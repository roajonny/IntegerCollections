...
//V.3 Includes a slimmer switch statement that only takes uppercase letters as arguments
//any selection made for the switch statement is passed into a function that makes makes it uppercase
//which is then used by the switch
...
bool checkEmpty(const vector<int>& int_storage)
{
    bool is_empty{ false };
...
void minNum(const vector<int>& int_storage)
{
...
void maxNum(const vector<int>& int_storage)
{
    bool is_empty = checkEmpty(int_storage);
    if(is_empty) {
...
void meanNum(const vector<int>& int_storage)
{
    int sum{};
    double mean{};
    bool is_empty = checkEmpty(int_storage);
...
void printNum(const vector<int>& int_storage)
{
    if(int_storage.size() == 0) {
...
void display_menu(vector<int>& int_storage)
{
    ...
    do {
        ...
        cout << " \nMake a selection: ";
        cin >> selection;
        selection = upper_case(selection);
        switch(selection) {
        case 'P': {
